...
class DCRNNModel(object):
    def __init__(self, is_training, batch_size, scaler, adj_mx, **model_kwargs):
        ...
        self._labels = tf.placeholder(tf.float32, shape=(batch_size, horizon, num_nodes*rnn_units, input_dim), name='labels')
        ...
        with tf.variable_scope('DCRNN_SEQ'):
            ...
            labels = tf.unstack(
                tf.reshape(self._labels[..., :output_dim], (batch_size, horizon, rnn_units*num_nodes * output_dim)), axis=1)
            ...
            attention_mechanism = tf.contrib.seq2seq.LuongAttention(num_nodes, attention_states)

            attention_cell = tf.contrib.seq2seq.AttentionWrapper(
                cell=decoding_cells, attention_mechanism=attention_mechanism,
            ...
            decoder_initial_state = attention_cell.zero_state(batch_size, dtype=tf.float32).clone(cell_state=enc_state)
            outputs, final_state = legacy_seq2seq.attention_decoder(labels, decoder_initial_state, attention_states ,attention_cell, loop_function=_loop_function)
            print("@@@@@@@@@@@@@@@@@@@@@@@@@")
            print(outputs)
        ...
        self._outputs = tf.reshape(outputs, (batch_size, horizon, num_nodes*rnn_units, output_dim), name='outputs')
