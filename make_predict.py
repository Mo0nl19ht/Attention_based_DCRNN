...
def run_dcrnn(args):
    ...
    nn=config['data']['dataset_dir']
    nn=nn[5:]
    path = f'prediction/{nn}_forvis_original'
    print(path)
    try:
        os.mkdir(path)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(12):
            ...
            origin.to_csv(f'{path}/after_{i + 1}h_origin.csv')
            predict.to_csv(f'{path}/after_{i+1}h_predict.csv')
